...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
        <div id="textt" name="textt">
            <p>Channeling List On <?php echo date("Y-m-d"); ?></p>
        </div>
        ...
        <div class="CSSTableGenerator" id="filt_table">  <table width="735" border="0" class=\"form-matrix-table\">
                <tr>
                    ...
                    <td width="176"  background=""><font color="#FFFFFF">Doctor</font></td>
                    <td width="426"  background=""><font color="#FFFFFF">Name</font></td>
                    <td width="176"  background=""><font color="#FFFFFF">Amount</font></td>
                </tr>
                <?php
                $sdate = date('Y-m-d');
                $sql = "SELECT * FROM pa_app_channel  where sdate = '" . $sdate . "' and cancell='0' order by serino desc  ";

                $result = mysqli_query($GLOBALS['dbinv'], $sql);
                while ($row = mysqli_fetch_array($result)) {

                    echo "<tr><td onclick=\"arnno_gin('" . $row['refno'] . "');\">" . $row['serino'] . "</a></td>";
                    echo "<td onclick=\"arnno_gin('" . $row['refno'] . "');\">" . $row["docnname"] . "</a></td>";
                    echo "<td onclick=\"arnno_gin('" . $row['refno'] . "');\">" . $row["paname"] . "</a></td>";
                    echo "<td onclick=\"arnno_gin('" . $row['refno'] . "');\">" . $row['cash'] . "</a></td>
                          </tr>";
                }
