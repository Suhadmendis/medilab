...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
        <div id="textt" name="textt">
            <p>Reg List On <?php echo date("Y-m-d"); ?></p>
        </div>
        ...
        <div class="CSSTableGenerator" id="filt_table">  <table width="735" border="0" class=\"form-matrix-table\">
                <tr>
                    ...
                    <td width="176"  background=""><font color="#FFFFFF">Tele</font></td>
                </tr>
                <?php
                 

                $sdate = date('Y-m-d');
                $sql = "SELECT * FROM pa_reg order by id desc  ";

                $result =  mysqli_query($GLOBALS['dbinv'], $sql);
                while ($row = mysqli_fetch_array($result)) {

                    echo "<tr><td onclick=\"arnno_gin('" . $row['refno'] . "','" . $_GET['stname'] ."');\">" . $row['refno'] . "</a></td>";

                    echo "<td onclick=\"arnno_gin('" . $row['refno'] . "','" . $_GET['stname'] ."');\">" . $row["paname"] . "</a></td>";

                    echo "<td onclick=\"arnno_gin('" . $row['refno'] . "','" . $_GET['stname'] ."');\">" . $row['tele'] . "</a></td>
