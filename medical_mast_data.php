...
////////////////////////////////////////////// Database Connector /////////////////////////////////////////////////////////////
	...
	/*		$sql="Select * from tmp_army_no where edu= '".$_GET['edu']."'";
		...
		if($_GET["Command"]=="new_inv")
		{
			...
			$sql = "SELECT * FROM tremas order by TCODE desc";
			$result=mysql_query($sql, $dbinv);
			$row = mysql_fetch_array($result);
    		$tmpinvno = ($row["TCODE"] + 1);
...
if($_GET["Command"]=="save_item")
{
	...
	$sql="delete from tremas where TCODE='".$_GET["code"]."'";
	$result=mysql_query($sql, $dbinv);
	
	
	$sql="insert into tremas (TCODE, TDESCRIPT, TAMOUNT) values ('".$_GET["code"]."', '".$_GET["name"]."', ".$_GET["down_pay"].")";
	$result=mysql_query($sql, $dbinv);
